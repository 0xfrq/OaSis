...
int fd_open(fd_table_t *table, const char *path, int flags) {
    ...
    if ((flags & O_RDWR) == O_RDWR) {
        vfs_flags = VFS_O_READ | VFS_O_WRITE;
    } else if (flags & O_WRONLY) {
        vfs_flags = VFS_O_WRITE;
    } else {
        vfs_flags = VFS_O_READ;
    }
    if (flags & O_CREAT) vfs_flags |= VFS_O_CREATE;
    ...
    entry->flags = 0;
    if (vfs_flags & VFS_O_READ) entry->flags |= FD_FLAG_READ;
    if (vfs_flags & VFS_O_WRITE) entry->flags |= FD_FLAG_WRITE;
    if (vfs_flags & VFS_O_APPEND) entry->flags |= FD_FLAG_APPEND;
    entry->ref_count = 1;
    entry->offset = 0;
    entry->data.vfs_fd = vfs_fd;
...
int fd_close(fd_table_t *table, int fd) {
    ...
    if (entry->type == FD_TYPE_FILE) {
        vfs_close(entry->data.vfs_fd);
    }
    /* Handle pipe cleanup */
    else if (entry->type == FD_TYPE_PIPE_READ && entry->data.pipe) {
...
int fd_read(fd_table_t *table, int fd, void *buf, uint32_t count) {
    ...
    switch (entry->type) {
        ...
        case FD_TYPE_FILE:
            return vfs_read(entry->data.vfs_fd, (char *)buf, count);
...
int fd_write(fd_table_t *table, int fd, const void *buf, uint32_t count) {
    ...
    switch (entry->type) {
        ...
        case FD_TYPE_FILE:
            return vfs_write(entry->data.vfs_fd, (const char *)buf, count);
...
int fd_seek(fd_table_t *table, int fd, int32_t offset, int whence) {
    ...
    switch (whence) {
        case SEEK_SET:
            ...
            break;
        case SEEK_CUR:
            new_offset = entry->offset + (uint32_t)offset;
            break;
        case SEEK_END:
            /* Would need to query file size from VFS - for now just use offset */
            new_offset = (uint32_t)offset;
            break;
        default:
            return -1;
    ...
    return vfs_seek(entry->data.vfs_fd, new_offset);
}
