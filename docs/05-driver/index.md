...
```c
static uint8_t keyboard_buffer[KEYBOARD_BUFFER_SIZE];  // 256 byte
...
tiap karakter 2 byte: char (low) + attribute (high).
...
64 entry cache untuk ngurangin akses disk langsung.

```c
typedef struct {
    uint32_t block_num;
    uint8_t data[BLOCK_SIZE];  // 512 byte
    int valid;
    int dirty;
} cache_entry_t;
